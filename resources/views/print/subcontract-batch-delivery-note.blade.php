...
<html>
<head>
    <title>Surat Jalan Subcont - Batch</title>
    <style>
        @page {
            margin: 0.5cm;
            size: A4;
...
<body>
    <div class="no-print" style="text-align: right; margin-bottom: 20px;">
        <button onclick="window.print()" style="padding: 10px 20px; background: #003680; color: white; border: none; cursor: pointer; border-radius: 5px; font-weight: bold;">PRINT BATCH SURAT JALAN</button>
    </div>
...
Telp : +62 21 89383915'))) !!}
            ...
            <td width="40%">
                <div class="doc-title">SURAT JALAN SUBCONT (BATCH)</div>
                <table style="margin-top: 10px; float: right;">
                    <tr>
                        <td class="font-bold" width="90">No. SJ (Ref)</td>
                        <td width="5" class="text-center">:</td>
                        <td>BATCH-{{ date('YmdHi') }}</td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="font-bold" width="90">Total WO</td>
                        <td width="5" class="text-center">:</td>
                        <td>{{ $orders->count() }} Orders</td>
                    </tr>
                </table>
            </td>
        </tr>
            ...
            <tr>
                <td width="50%">
                    ...
                    <div style="padding-left: 10px; border-left: 2pt solid #003680;">
                        <span class="font-bold" style="font-size: 11pt;">{{ $supplier->name }}</span><br>
                        {!! nl2br(e($supplier->address)) !!}<br>
                        Telp: {{ $supplier->phone }}
                    </div>
                </td>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            @foreach($orders as $order)
                @if(isset($movementItemsByOrder[$order->id]))
                    @php 
                        $itemCount = count($movementItemsByOrder[$order->id]); 
                        $i = 0; 
                    @endphp
                    @foreach($movementItemsByOrder[$order->id] as $mv)
                    <tr>
                        @if($i == 0)
                        <td class="text-center" rowspan="{{ $itemCount }}" style="vertical-align: middle;">{{ $globalIndex++ }}</td>
            ...
            <td width="75%" style="vertical-align: top;">
                <div style="border: 0.5pt solid #000; padding: 10px;">
                    ...
                    <div style="height: 60px; font-style: italic; color: #555;">
                        Mohon barang diproses sesuai perhitungan qty Barang Jadi . Jika terjadi kerusakan material dalam proses di subcont harap dilaporkan segera. (Batch Document)
                    </div>
                </div>
            </td>
            <td width="25%" style="text-align: right; vertical-align: top;">
                <!-- QR Code for the first order as validation fallback -->
                @if($orders->count() > 0)
                <div class="qr-box" style="float: right;">
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data={{ urlencode(route('manufacturing.subcontract-orders.public-validate-sj', $orders->first()->id)) }}" class="qr-image">
    ...
    <div class="signatures-section">
        <table class="sig-table">
            <tr>
                ...
                <td>
                    Subcontractor,
                    <div class="sig-box"><div class="sig-label">( {{ $supplier->name }} )</div></div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div style="margin-top: 20px; font-size: 8pt; color: #888; text-align: center; border-top: 0.5pt solid #eee; padding-top: 5px;">
        Dokumen batch ini dicetak otomatis oleh sistem ERP JIDOKA pada {{ date('d/m/Y H:i') }}
    </div>
</body>
</html>
