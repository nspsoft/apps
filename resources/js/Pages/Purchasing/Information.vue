            ...
            <div class="max-w-7xl mx-auto relative z-10">
                ...
                <div v-else-if="activeTab === 'manual'" class="max-w-5xl mx-auto space-y-8">
                    <div class="text-center mb-8 animate-in fade-in duration-700">
                        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-widest mb-4"><ListIcon class="w-4 h-4" /> Comprehensive Guide</div>
                        <h1 class="text-4xl font-bold mb-4 text-slate-900 dark:text-white">Panduan Modul Purchasing</h1>
                        <p class="text-slate-500 dark:text-slate-400">Dokumentasi lengkap operasional fitur pembelian.</p>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div v-for="(section, idx) in manualSections" :key="section.id" :id="'manual-' + section.id" class="animate-in slide-in-from-bottom-3 duration-500" :style="{ animationDelay: idx * 50 + 'ms' }">
                            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[20px] overflow-hidden transition-all duration-300 hover:shadow-lg" :class="expandedSections.has(section.id) ? 'shadow-xl ring-1 ring-blue-500/20' : ''">
